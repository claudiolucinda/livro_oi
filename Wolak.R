...
data_MW_Producer=data_Excel %>%
  group_by(OWNER) %>%
  summarise(capacity=sum(MW*(1-NERC_FOF))) %>%
...
teste_1998=data_1998 %>%
  left_join(data_1998_CAL, by=c("Year","Month","Day","Hour")) %>%
  filter(Price>20) %>%
...
teste_1999=data_1999 %>%
  left_join(data_1999_CAL, by=c("Year","Month","Day","Hour")) %>%
  filter(Price>20) %>%
  mutate(Q1=Quantity-Qinstate+data_MW_Producer$capacity[1],
         Q2=Quantity-Qinstate+data_MW_Producer$capacity[2],
         Q3=Quantity-Qinstate+data_MW_Producer$capacity[3],
...
teste_2000=data_2000 %>%
  left_join(data_2000_CAL, by=c("Year","Month","Day","Hour")) %>%
  filter(Price>20) %>%
  mutate(Q1=Quantity-Qinstate+data_MW_Producer$capacity[1],
         Q2=Quantity-Qinstate+data_MW_Producer$capacity[2],
         Q3=Quantity-Qinstate+data_MW_Producer$capacity[3],
