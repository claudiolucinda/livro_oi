...
# Testando fazer a oferta competitiva
data_STATA %>%
  filter(year==2000, month>5 & month<10) %>%
  ggplot(aes(Qinstate, MC)) +
  geom_point() +
  stat_smooth(method='lm', formula = y~poly(x,3))


data_STATA98=data_STATA %>%
  filter(year==1998, month>5 & month<10)

model_98_CSupply=loess(Qinstate~MC, data=data_STATA98)
...
model_98_CSupply=lm(Qinstate~poly(MC,3), data=data_STATA98)

data_STATA99=data_STATA %>%
  filter(year==1999, month>5 & month<10)

model_99_CSupply=loess(Qinstate~MC, data=data_STATA99)

data_STATA00=data_STATA %>%
  filter(year==2000, month>5 & month<10)

model_00_CSupply=loess(Qinstate~MC, data=data_STATA00)
...
# testing
test=data.frame(data_1998$Price)
colnames(test)="MC"
Q_S98=predict(model_98_CSupply, newdata = test)
data_1998$CSupply=Q_S98
data_1998$CSupply=ifelse(data_1998$Price<20,635,data_1998$CSupply)
data_1998$CSupply=ifelse(data_1998$CSupply<0,15633,data_1998$CSupply)

data_1998=data_1998%>%
  group_by(Year, Month, Day, Hour) %>%
  arrange(Year, Month, Day, Hour, Price) %>%
  mutate(Qty1=Quantity-CSupply+data_MW_Producer$capacity[1],
         Qty2=Quantity-CSupply+data_MW_Producer$capacity[2],
         Qty3=Quantity-CSupply+data_MW_Producer$capacity[3],
         Qty4=Quantity-CSupply+data_MW_Producer$capacity[4]) %>%
  mutate(PH=lead(Price), QH=lead(Qty1), PL=lag(Price), QL=lag(Qty1)) %>%
  mutate(arcelast=((QH-QL)/(PH-PL))*((PH+PL)/(QH+QL)))
 summary(data_1998$arcelast)
