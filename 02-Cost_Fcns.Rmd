...
```{r cost_fig, echo=FALSE, engine='tikz', fig.ext='svg'}
\usetikzlibrary{intersections}
\begin{tikzpicture}[samples=32, >=latex]
\draw[->] (0,0) -- (3.5,0) node [right] {$q$};
\draw[->] (0,0) -- (0,2.5) node [above] {$CT$};
\draw[name path=LRTC, color=green, thick, domain=0:3] plot (\x, {0.8*\x*\x*\x-3.4*\x*\x+4*\x}) node[right]{CTLP($q$)};
\end{tikzpicture}

```


```{r cost_fig02, echo=FALSE, engine='tikz', fig.ext='svg'}
\usetikzlibrary{intersections}
\begin{tikzpicture}[samples=32, >=latex]
\draw[->] (0,0) -- (3.5,0) node [right] {$q$};
...
```{r cost_fig03, echo=FALSE, engine='tikz', fig.ext='svg'}
\usetikzlibrary{intersections}
\begin{tikzpicture}[samples=32, >=latex]
\draw[->] (0,0) -- (3.5,0) node [right] {$q$};
\draw[->] (0,0) -- (0,2.5) node [above] {LRTC};
...
\fill (intersection-1) circle(1pt) node[right]{$A^{\prime}$};
\path[name intersections={of=vert2 and LRMC}];
\fill (intersection-1) circle(1pt) node[right]{$A^{\prime \prime}$};
\end{tikzpicture}
```
